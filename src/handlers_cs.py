...
async def handle_cs(plugin, event) -> AsyncIterator:
    ...
    if not raw:
        names = plugin.cs_store.list_names(user_id)
        if not names:
    ...
    if plugin.cs_store.exists(user_id, name):
    ...
    if nn_prompt:
        ...
        try:
            cssaying = plugin.cs_store.load_cssaying()
    ...
    try:
        plugin.cs_store.write(user_id, name, content, overwrite=False)
...
async def handle_dcs(plugin, event) -> AsyncIterator:
    ...
    user_id = plugin._get_user_id(event)
    deleted = plugin.cs_store.delete(user_id, name)
    if deleted:
...
async def handle_scs(plugin, event) -> AsyncIterator:
    ...
    user_id = plugin._get_user_id(event)
    if not plugin.cs_store.exists(user_id, name):
        yield event.plain_result(f"角色保持 {name} 不存在")
        return

    content = plugin.cs_store.read(user_id, name)
...
async def handle_ccs(plugin, event) -> AsyncIterator:
    ...
    plugin.cs_store.write(user_id, name, updated, overwrite=True)
