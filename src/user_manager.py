...
class UserManager:
    ...
    def _save(self):
        ...
        if self._store is None:
            return
        self._ensure_dir()
        self.data_file.write_text(self._store.model_dump_json(indent=2), "utf-8")
    ...
    def checkin(self, user_id: str, config: "Config") -> tuple[bool, int, str]:
        ...
        if user.quota >= checkin_limit:
            ...
            return (
                True,
                0,
                f"签到成功！但你的额度已达到签到上限({checkin_limit}次)，本次获得 0 次画图机会",
            )
        ...
        return (
            True,
            gained,
            f"签到成功！获得 {gained} 次画图机会，当前剩余 {user.quota} 次",
        )
