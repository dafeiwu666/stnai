...
COMMAND = "nai"
PLUGIN_NAME = "astrbot_plugin_stnai"
...
class STNaiGenerateImageTool(ConfigNeededTool):
    ...
    async def call(
        ...
        **kwargs,
        ...
        vision_images = [
            img for idx, img in enumerate(images) if idx not in used_indices
        ]
...
class Plugin(Star):
    ...
    def __init__(self, context: Context, config: AstrBotConfig):
        ...
        # 初始化用户管理器和预设管理器，数据存储在共享插件 data 目录（与 ppnai 保持一致）
    ...
    async def initialize(self):
        ...
        # 兼容旧格式，尽量把已持久化的自动画图状态补齐为新结构
        for umo, state in list(self.auto_draw_info.items()):
            if not state:
                continue
            state.setdefault("presets", [])
            state.setdefault("cs_names", [])
            state.setdefault("vibe_transfer_images", [])
            ...
            for component in event.message_obj.message:
                if isinstance(component, Reply):
                    ...
                    if hasattr(event.message_obj, "reply") and event.message_obj.reply:
                        ...
                        if hasattr(reply_msg, "message") and isinstance(
                            reply_msg.message, str
                        ):
                            return reply_msg.message
                        if hasattr(reply_msg, "text") and isinstance(
                            reply_msg.text, str
                        ):
    ...
    async def _parse_args(
        ...
        for line in lines:
            ...
            if "=" in line:
                ...
                if preset_num:
                    ...
                    if preset is None:
                        raise ValueError(
                            f"预设 {value} 不存在，使用 nai预设列表 查看可用预设"
                        )
                ...
                # 强制键值对格式，不接受无等号的行
                raise ValueError(
                    f"参数格式错误：'{line}'，请使用键值对格式，例如：tag=xxx"
                )
        ...
        sorted_presets = sorted(
            zip(preset_numbers, preset_params_list), key=lambda x: x[0]
        )
        ...
        req = await parse_req(
            final_raw, event.message_obj.message, self.config, is_whitelisted
        )
    ...
    async def on_llm_response_auto_draw(
